            ...
            <div className="flex items-center justify-between p-4">
                <div className="flex items-center gap-3">
                    <button
                        ...
                        aria-label={collapsed ? 'Expand Get Started' : 'Collapse Get Started'}
                        onClick={() => setCollapsed(!collapsed)}
                        className="w-8 h-8 flex items-center justify-center text-muted-foreground hover:text-foreground transition-colors"
                    >
                        {collapsed ? '▾' : '▴'}
                    </button>
                    <h2 className="text-xl font-semibold m-0">Get Started</h2>
                </div>
                {/* when collapsed we show a very compact inline status */}
                <div className="mr-2">
                    {collapsed ? <StatusInline /> : null}
                </div>
            </div>
            {!collapsed && (
                <div className="p-6">
